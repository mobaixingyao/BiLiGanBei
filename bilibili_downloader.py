...
class _DanmakuLayout:
    ...
    def __init__(self, play_res_x, play_res_y, font_size=20):
        self.play_res_x = play_res_x
        self.play_res_y = play_res_y
        self.font_size = font_size
        self.track_height = font_size + 1  # 轨道间距: 字号+1 (参照样例 21=20+1)
    ...
    def _find_track(self, tracks, start_time, duration, y_start, max_tracks):
        ...
        for i in range(max_tracks):
            ...
            conflict = False
            for t_end, t_y in tracks:
                if t_y == y and t_end > start_time:
                    conflict = True
                    break
            if not conflict:
    ...
    def get_scroll_y(self, start_time, duration):
        """滚动弹幕 y 坐标 (从上往下排)"""
        max_tracks = max(1, (self.play_res_y - self.track_height) // self.track_height)
...
def _danmaku_list_to_ass(danmaku_list, play_res_x=560, play_res_y=420, font_size=20,
                         scroll_duration=16.0, fix_duration=4.0):
    """将弹幕列表转换为 ASS 字幕格式。
    样式参照 B站官方 ASS 导出: 黑体半透明、Fix+R2L 双样式、轨道碰撞检测。
    ...
    layout = _DanmakuLayout(play_res_x, play_res_y, font_size)
...
Style: Fix,黑体,{font_size},&H80FFFFFF,&H80FFFFFF,&H80000000,&H80000000,0,0,0,0,100,100,0.00,0.00,1,1.00,0.00,2,30,30,30,0
Style: R2L,黑体,{font_size},&H80FFFFFF,&H80FFFFFF,&H80000000,&H80000000,0,0,0,0,100,100,0.00,0.00,1,1.00,0.00,2,30,30,30,0
...
class BilibiliClient:
    ...
    def get_danmaku(self, cid, bvid="", fmt="ass", max_segments=100):
        ...
        return _danmaku_list_to_ass(all_danmaku)
